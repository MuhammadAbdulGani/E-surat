            ...
            <div class="card-body">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap"
                    ...
                    <tbody>
                        ...
                        @forelse(PendingSubmissions()->get() as $submission)
                        <tr>
                            ...
                            <td>
                                ...
                                @if (!CheckRole::PT())
                                <form method="POST"
                                        action="{{ route('admin.submissions.status', [$submission->id, 'approve']) }}"
                                        class="d-inline form-patch">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-sm btn-primary waves-effect waves-light {{(CheckRole::PN() || CheckRole::PT()) ?? 'd-none'}}"><i
                                                class="mdi mdi-format-horizontal-align-right"></i> Setujui</button>
                                    </form>
                                    <form method="POST"
                                        action="{{ route('admin.submissions.status', [$submission->id, 'reject']) }}"
                                        class="d-inline form-patch">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-sm btn-danger waves-effect waves-light {{(CheckRole::PN() || CheckRole::PT()) ?? 'd-none'}}"><i
